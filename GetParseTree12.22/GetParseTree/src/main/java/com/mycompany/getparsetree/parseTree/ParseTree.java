package com.mycompany.getparsetree.parseTree;

import com.mycompany.getparsetree.Bmcl;
import com.mycompany.getparsetree.Vocabulary;
...
int aabbcc = Bmcl.functionNameToIndex.get(Bmcl.OwnMethodName);
	...
	public void DFS(ParseTreeNode node, int n, int father,int level){//深度优先遍历
		int index = ++number;
...
//System.out.println(aabbcc + " " + Bmcl.OwnMethodName + " " + node.getValue());
	...
	Bmcl.abc3++;
			...
			if (node.getChildren().size() == 0)//没有孩子了
